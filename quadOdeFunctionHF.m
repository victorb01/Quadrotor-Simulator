...
rIdot = vI;
vIdot = [0; 0; -P.constants.g] + transpose(RBI)*sumF/P.quadParams.m + distVec/P.quadParams.m - aeroD(transpose(RBI)*[0; 0; 1], vI, P)/P.quadParams.m;
RBIdot = -crossProductEquivalent(omegaB)*RBI;
RBIdotVec = RBIdot(:);
omegaBdot = P.quadParams.Jq\(sumN + sumNF - crossProductEquivalent(omegaB)*P.quadParams.Jq*omegaB);
omegaVecDot = (P.quadParams.cm.*eaVec - omegaVec)./P.quadParams.taum;

%Make the final output
Xdot = [rIdot; vIdot; RBIdotVec; omegaBdot; omegaVecDot];
end